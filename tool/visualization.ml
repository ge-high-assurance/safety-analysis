...
(**
   ...
   let nasa_cutsets = mcutsets nasa_ftree;;
...
let dot_core_gen_show_file ?(rend="svg") ?(splines="true") ?(layout="dot")
    ?(overlap="false") ?(unflatten=true) file
    (f : ?splines:string -> ?layout:string -> ?overlap:string -> ?chan:out_channel -> unit)
    =  
  let chan = open_out file in
...
(** Code to construct the graphviz file and display it.  For this to work, you
    ...
    should work). I am using "open" to view the .svg file generated, so if you're
    using this on a non-mac, define a version of this for your OS as a separate
    function.
*)
...
let dot_gen_show_direct_tree_file ?(rend="svg") ?(splines="true") ?(layout="dot")
    ?(overlap="false") ?(unflatten=true) file tree =
...
let dot_gen_show_formula_file ?(rend="svg") ?(splines="true") ?(layout="dot")
    ?(overlap="false") ?(unflatten=true) file formula =
  dot_core_gen_show_file ~rend ~splines ~layout ~overlap ~unflatten file
...
let fault_dot ?(es=[]) lib model chan =
  ...
  let edges = List.filter gedges ~f:(fun x -> (not (List.mem es ~equal:(=) x)))  in 
...
let gen_fault_f_edges library instances connections nodes f ff  =
  ...
  let es =
	...
	let (l1,l2) =
	  if (List.mem c.input_flows ~equal:(=) xh)
 	  then
	    let ((s,p), (a1,b1)) =
...
let rec cons_fault_edges library instances connections nodes his fl res =
  match fl with
	...
	library instances connections nodes f fmx in
      let nedges = List.filter edges ~f:(fun x -> not (List.mem res ~equal:(=) x)) in
      let nfaults = List.filter nextfaults
	~f:(fun x -> not (List.mem his ~equal:(=) x)) in
      cons_fault_edges
