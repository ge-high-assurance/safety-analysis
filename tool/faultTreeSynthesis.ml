...
let filter_v l bound connections inst acc =
  List.filter l
	...
	let nform = [o; flt] in
	let idx = (n, nform) in
	let v = if List.Assoc.mem acc ~equal:(=) idx
	  then (List.Assoc.find_exn acc ~equal:(=) idx) 
	  else 0 in
...
let add_to_acc_expand_form i acc =
  let mem = List.Assoc.mem acc ~equal:(=) i in
  let v = if mem
    then (List.Assoc.find_exn acc ~equal:(=) i) 
    else 0 in
  if mem
  then (i, v + 1)::(List.Assoc.remove acc ~equal:(=) i)
  else (i, 1)::acc ;;
